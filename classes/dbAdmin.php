...
class dbAdmin {
    ...
    public function getCotizacionesByCustomFilterMerge($params) {
    ...
    public function getCotizacionesWithNoParentCompany() {
    ...
    public function updateCotHeaderByMergeCot($params) {
            
            $list = json_decode(stripslashes($params['list']));

            $sql ="update cotz_header set";

            if( isset($params['companyId']) && $params['companyId'] != '' &&
                isset($params['contactId']) && $params['contactId'] != ''){
                $sql .= " account_id = ". $params['companyId'];
                $sql .= " ,contact_id = ". $params['contactId'];
                
                if( isset($params['vendedorId']) && $params['vendedorId'] != ''){
                    $sql .= " , ";
                }
            }
            if( isset($params['vendedorId']) && $params['vendedorId'] != ''){
                $sql .= " vendedor_id = ". $params['vendedorId'];
            }
            
            $sql .= " where id in (".join(' , ', $list).")";
